...
use rom_pipeline_core::{
    CompletionRecord, Job, JobOutcome, PipelineAdapter, PipelineError, ProfileConfig, Ps2Settings,
    Readiness, Result, StateStore, StopToken, completion_output_valid, modified_seconds,
};

use crate::inventory::Ps2Inventory;
...
impl Ps2Adapter {
    /// Creates a validated PS2 adapter.
    ///
    /// # Errors
    ///
    ...
    pub fn new(profile: ProfileConfig) -> Result<Self> {
        profile.validate()?;
        if !matches!(profile.system, rom_pipeline_core::SystemKind::PlayStation2) {
    ...
    pub(crate) fn settings(&self) -> Result<&Ps2Settings> {
        self.profile
            .ps2
            .as_ref()
    ...
    pub fn preflight(&self) -> Result<()> {
        ...
        if !self
            .profile
            .source_format
            .eq_ignore_ascii_case("disc-image")
            || !self.profile.output_format.eq_ignore_ascii_case("mixed")
        {
            return Err(PipelineError::InvalidConfig(
                "PS2 conversion requires source_format=disc-image and output_format=mixed"
                    .to_owned(),
            ));
        }
    ...
    pub(crate) fn move_sources_to_done(&self, job: &Job, state: &StateStore) -> Result<()> {
        ...
        for artifact in &job.sources {
            ...
            match (source.is_file(), done.is_file()) {
                (true, true) => {
                    return Err(PipelineError::Message(format!(
                        "PS2 source exists in source and done: {}",
                        artifact.name
                    )));
                }
                (true, false) => {
                    ...
                    state.log(&format!("MOVED PS2 source to done: {}", artifact.name))?;
                }
                (false, true) => {}
...
impl PipelineAdapter for Ps2Adapter {
    fn inventory(&self, only_job: Option<&str>) -> Result<Vec<Job>> {
        Ok(Ps2Inventory::from_manifest(
